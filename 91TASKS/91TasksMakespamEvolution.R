...
# Read in data
df <- read.csv("C:/Users/Jorge/Desktop/TFG 10 NOV/91TASKS/91TasksMakespamEvolution.txt", stringsAsFactors = F)
...
for(i in 1:(nrow(df))){
  ...
  p <- add_trace(p,
                 ...
                 y = c(df$MakespamInIteration[i], df$MakespamInIteration[i]),  # y0, y1
                 mode = "lines",
