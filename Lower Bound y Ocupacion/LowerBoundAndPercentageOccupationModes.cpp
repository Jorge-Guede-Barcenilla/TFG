...
/*DECLARACION DE VARIABLES*/
	int main() 													{	
		...
		int NumberOfTasks=204, SolutionIterations=0, NumberOfAreas=0, DesiredSolutions=20000, NumberOfProfiles=0, Area1Capacity=0, Area2Capacity=0, Area3Capacity=0, k=0, j=0, i=0, m=0, l=0, Aux1=0, Aux2=0, Aux3=0, Aux4=0, Aux5=0, Aux6=0, Aux7=0, Aux8=0,
		...
		TotalOperatorsOfProfile1=4, TotalOperatorsOfProfile2=2, OperatorsInArea1[NumberOfTasks][aux], OperatorsInArea2[NumberOfTasks][aux], MinimumOperators[NumberOfTasks], MaximumOperators[NumberOfTasks],
		...
		AuxiliarCounter1=0, AuxiliarCounter2=0, AuxiliarCounter3=0,
		 PercentageLimit=0, ModeSelection=0;
		float  LowerBound=0, TotalHoursOfWork=0, CicleCharge=0,	StartTime=clock(),	Area1_1TotalOccupancy=0, Area1_2TotalOccupancy=0, Area1_3TotalOccupancy=0, Area1_4TotalOccupancy=0, Area1_5TotalOccupancy=0, Area2_1TotalOccupancy=0, Area2_2TotalOccupancy=0, Area2_3TotalOccupancy=0;
	...
	TECLEEOTRAVEZ:													
	printf("Teclee 1 si desea emplear la restriccion Frontera Inferior o bien 2 para la restriccion Porcentaje de Ocupacion Frontera\n");
...
if(ModeSelection!=1&&ModeSelection!=2) 	{
	...
	TECLEE:										
	printf("Introduzca numero de operarios de tipo Mecanico y de tipo Estructural\n");
	scanf("%d %d", &TotalOperatorsOfProfile1, &TotalOperatorsOfProfile2);

if(TotalOperatorsOfProfile1<4||TotalOperatorsOfProfile2<2)		{
	
	printf("Teclee un numero mayor de operarios\n");
	goto TECLEE;
																}

if(ModeSelection==1)											{

	printf("Introduzca ciclo\n");
	scanf("%f", &CicleCharge);	
...
if(ModeSelection==2)											{
	...
	printf("Introduzca porcentaje\n");
	scanf("%d", &PercentageLimit);	

																}	
																
float ExecutionTime=0;
/*AQUI COMIENZA A CONTAR EL TIEMPO*/																											
...
/*YA TENEMOS TODOS LOS SUCESORES (SIN REPETIRSE), AHORA LOS SUMAMOS*/
	...
	GRPW[i]+=Duration[GRPWSuccessors[i][m]][0]*(OperatorsOfProfile1[GRPWSuccessors[i][m]][0]+OperatorsOfProfile2[GRPWSuccessors[i][m]][0]);	
	m++;
...
/*INICIALIZO*/											
	...
	State[123]=1, State[140]=1, State[153]=1, State[162]=1, State[164]=1, State[195]=1, TotalHoursOfWork=0;
	AuxiliarArea1_1Occupancy=0, AuxiliarArea1_2Occupancy=0, AuxiliarArea1_3Occupancy=0, AuxiliarArea1_4Occupancy=0, AuxiliarArea1_5Occupancy=0, AuxiliarArea2_1Occupancy=0, AuxiliarArea2_2Occupancy=0, AuxiliarArea2_3Occupancy=0;
	Area1_1Occupancy=0, Area1_2Occupancy=0, Area1_3Occupancy=0, Area1_4Occupancy=0, Area1_5Occupancy=0, Area2_1Occupancy=0, Area2_2Occupancy=0, Area2_3Occupancy=0;
...
for(i=0; i<NumberOfTasks+1; i++)							{
		...
		TotalHoursOfWork+=Duration[i][0]*(OperatorsOfProfile1[i][0]+OperatorsOfProfile2[i][0]);
...
LowerBound=TotalHoursOfWork/CicleCharge;

if (LowerBound - (int)LowerBound > 0) {
    LowerBound += 1;
...
for(k=0; k<NumberOfTasks; k++)	
	...
	if(ModeSelection==1&&(TotalHoursOfWork/Makespam<=LowerBound))	


						
										{
		printf("\n\nThe solution is obtained through the extra restriction mode\n");									
		printf("Total number of hours is %f\n", TotalHoursOfWork);	
		printf("Total operators of profile 1 is %d\n", 	TotalOperatorsOfProfile1);	
		printf("Total operators of profile 2 is %d\n", 	TotalOperatorsOfProfile2);
		printf("Lower Bound is: %f and total theorical operators obtained are %f\n", LowerBound, TotalHoursOfWork/Makespam);
	...
	if(ModeSelection==2&&(TotalHoursOfWork/((TotalOperatorsOfProfile1+TotalOperatorsOfProfile2)*Makespam))*100>PercentageLimit)
		...
		printf("Total operators of profile 1 is %d\n", 	TotalOperatorsOfProfile1);	
		printf("Total operators of profile 2 is %d\n", 	TotalOperatorsOfProfile2);
		printf("Percentage limit is: %d and percentage obtained is %f\n", PercentageLimit, (TotalHoursOfWork/((TotalOperatorsOfProfile1+TotalOperatorsOfProfile2)*Makespam))*100);
